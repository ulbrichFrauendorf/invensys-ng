 ...
 * ```
 */
interface IChartLegendItem extends LegendItem {
...
export class IChart implements AfterViewInit, OnDestroy, OnChanges {
  ...
  /**
   * Legend items generated from Chart.js for each chart instance.
   * @internal
   */
  ...
  /**
   * Get legend items for a chart display.
   * @param chartIndex - The index of the chart display
   * @returns Legend items generated by Chart.js
   */
  getLegendItems(chartIndex: number): IChartLegendItem[] {
    return this.chartLegendItems[chartIndex] ?? [];
  }

  /**
   * Toggle a chart legend item using the same Chart.js behavior as the built-in legend.
   * @param chartIndex - The index of the chart display
   * @param item - The legend item to toggle
   */
  toggleLegendItem(chartIndex: number, item: IChartLegendItem): void {
    ...
    chart.update();
    this.updateLegendItems(chartIndex);
  }

  /**
   * Refresh the custom legend items for a chart instance.
   * @internal
   */
  private updateLegendItems(chartIndex: number): void {
    const chart = this.chartInstances[chartIndex];
    const generateLabels =
      chart?.options.plugins?.legend?.labels?.generateLabels;

    if (!chart || !generateLabels) {
      this.chartLegendItems[chartIndex] = [];
      return;
    }

    this.chartLegendItems[chartIndex] = generateLabels(chart).map((item) => ({
      ...item,
